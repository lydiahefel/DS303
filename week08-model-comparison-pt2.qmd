...
anova(uniforms_long, uniforms_2)
anova(uniforms_long, uniforms_3)
anova(uniforms_long, uniforms_4)
anova(uniforms_long, uniforms_5)
anova(uniforms_long, uniforms_6)
...
```{r}
m.best_int <- lm(mpg ~ wt + qsec + am + cyl:hp, data = mtcars)
summary(m.best_int)

anova(m.best, m.best_int)
```

In plain language, what does your interaction mean for predicting `mpg`? Does it improve the model?

-   How does the interaction between `hp` (horsepower) and `cyl` (number of cylinders) help predict `mpg`?

-   The interaction between `hp` and `cyl` is not statistically significant, so it does not improve the model.
