...
4.  What are the limitations of this coding method? Are there situations where this method may lead to misleading results or interpretations?
    ...
    A limitation of simple coding is that the reference level is a variable that may not always be an actual baseline, only because it came first alphabetically or numerically. The contrast matrix is a bit more confusing than dummy coding. Each row reprensents one level of the original variable and there's k-1 columns. However, the matrix columns use -1/k and (k-1)/k values to avoid collinearity, to maintain that the contrast sums to zero and to keep the intercept the grand mean. A non-zero reference level helps to shift the intercept to the overall mean rather than the reference mean.
...
Summarize these findings. What is your reference group, and how do the other groups compare with regards to their writing scores? What does the intercept value represent?\
\
The reference group is `race.f1`, the other groups writing scores are all higher in comparison to the reference group.

For every one unit increase in `race.f2`, the writing score is roughly 11.54 points higher than `race.f1`.

For every one unit increase in `race.f3`, the writing score is roughly 1.74 points higher than `race.f1`.

For every one unit increase in `race.f4`, the writing score is roughly 7.59 points higher than `race.f1`.

The `intercept` value of 51.68 represents the overall grand mean.

The mean of the reference level (i.e. `race.f1`) is not listed in the output.
