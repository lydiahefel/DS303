...
-   crim - The coefficient for `crim` is -0.121, indicating that for each one-unit increase in `crim`, the median home value decreases by about 0.121 units. Crime rate goes up, price goes down.

-   indus - The coefficient for `indus` is 0.013, indicating that for each one-unit increase in `indus`, the median home value increases by about 0.013 units. However, the high p-value indicates that `indus` is not statistically significant.

-   tax - The coefficient for `tax` is -0.013, indicating that for each one-unit increase in `tax`, the median home value decreases by about 0.013 units.

**Model Fit**:

**Do you think this is a good model to use for predicting median home values in Boston?**

-   The model seems to be a relatively good fit with around 73% accuracy. There are a lot of statistically significant variables in the data set that would seemingly have an impact on the housing market in Boston. However, it was interesting to see that the age variable was not as statistically significant as one would assume.
